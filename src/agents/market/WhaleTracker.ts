...
export class WhaleTracker extends BaseAgent {
  ...
  protected async performAnalysis(context: AgentContext): Promise<WhaleTrackerResult> {
    ...
    const [largeTransactions, whaleWallets, exchangeFlows, smartMoney, accumulationPatterns] =
      await Promise.all([
  ...
  private determineAction(txs: any[], patterns: any[]): WhaleMovement['action'] {
    ...
    const hasAccumulationPattern = patterns.some(
      (p) => p.type === 'accumulation' && txs.some((tx) => tx.hash === p.transactionHash),
    );
  ...
  private calculateMarketImpact(movements: WhaleMovement[], exchangeFlows: any): MarketImpact {
    let accumulation = 0;
    let distribution = 0;
  ...
  private generateStatistics(movements: WhaleMovement[], transactions: any[]): WhaleStatistics {
    const uniqueWhales = new Set(movements.map((m) => m.wallet.address));
    const activeWhales = movements.filter(
      (m) => m.action === 'ACCUMULATION' || m.action === 'DISTRIBUTION',
    ).length;
    ...
    const avgTransaction =
      txValues.length > 0 ? txValues.reduce((sum, val) => sum + val, 0) / txValues.length : 0;
