...
export abstract class BaseAgent {
   ...
   */
  protected abstract performAnalysis(context: AgentContext): Promise<unknown>;

  /**
   * Abstract method for validating agent-specific input
   */
  protected abstract validateInput(context: AgentContext): z.ZodSchema;
  ...
  public async analyze(context: AgentContext): Promise<AnalysisResult> {
    ...
    try {
      ...
      // Convert to OnChainAgentError for proper handling
      const agentError =
        error instanceof OnChainAgentError
          ? error
          : new OnChainAgentError(
              error instanceof Error ? error.message : 'Unknown error',
              error instanceof z.ZodError ? ErrorType.VALIDATION : ErrorType.INTERNAL,
              ErrorSeverity.MEDIUM,
              {
                context: { agent: this.config.name, ...context },
  ...
  protected async withRetry<T>(fn: () => Promise<T>, retries: number): Promise<T> {
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        const isRetryable = error instanceof OnChainAgentError ? error.retryable : true; // Default to retryable for unknown errors
