...
export class DeFiAnalyzer extends BaseAgent {
  ...
  private identifyYieldOpportunities(
    ...
    for (const yield_ of yieldData) {
      ...
      // Filter by risk
      const maxRisk = context.options?.maxRisk || 10;
      ...
      opportunities.push({
        ...
        asset: yield_.asset || context.asset || 'Unknown',
  ...
  private assessSmartContractRisk(protocols: ProtocolAnalysis[]): RiskMetric {
    ...
    const avgAuditScore = protocols.reduce((sum, p) => sum + p.auditStatus.score, 0) / protocols.length;
  ...
  private assessLiquidityRisk(liquidityData: any, opportunities: YieldOpportunity[]): RiskMetric {
  ...
  private assessProtocolRisk(protocols: ProtocolAnalysis[], historicalData: any): RiskMetric {
    ...
    const unsustainableCount = protocols.filter(p => 
      p.yieldSustainability === 'UNSUSTAINABLE'
    ).length;
  ...
  private generateRecommendations(
    ...
    if (opportunities.length > 0 && context.amount && context.amount > 10000) {
      recommendations.push({
        type: 'ACTION',
        priority: 'HIGH',
  ...
  private createAllocationStrategy(
    ...
    const totalAmount = context.amount || 10000;
    ...
    const rebalanceFrequency = this.determineRebalanceFrequency(strategy);
  ...
  private calculateAllocations(
    ...
    for (const opp of eligibleOpps) {
      const allocationPercent = this.calculateAllocationPercent(
        opp,
        strategy,
        maxPercent
      );
  ...
  private optimizeGas(allocations: Allocation[], gasData: any): GasStrategy {
    ...
    const optimalChain = Array.from(chainCounts.entries())
      .sort((a, b) => b[1] - a[1])[0]?.[0] || 'arbitrum';
  ...
  private determineRebalanceFrequency(strategy: string): string {
    switch (strategy) {
      case 'conservative': return 'Quarterly';
      case 'balanced': return 'Monthly';
      case 'aggressive': return 'Weekly';
