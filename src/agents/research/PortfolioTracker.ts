...
export class PortfolioTracker extends BaseAgent {
  ...
  protected async performAnalysis(context: AgentContext): Promise<PortfolioTrackerResult> {
    ...
    const [balances, transactions, prices, stakingPositions, defiPositions, historicalData] =
      await Promise.all([
    ...
    const performance = this.calculatePerformance(holdings, transactions, historicalData);

    // Process recent transactions
    const recentTransactions = this.processTransactions(transactions, prices, context);

    // Generate insights
    const insights = this.generateInsights(portfolio, holdings, performance, recentTransactions);
  ...
  private async getBalances(context: AgentContext): Promise<any[]> {
    const chains = (context.options?.chains as string[] | undefined) || [
      'ethereum',
      'bsc',
      'polygon',
      'arbitrum',
    ...
    const balancePromises = chains.map((chain: string) =>
    ...
    return results.flatMap((r: any) => r.data as any[]);
  }
  ...
  private async getCurrentPrices(_context: AgentContext): Promise<Map<string, number>> {
  ...
  private processHoldings(
    ...
    balances.forEach((balance) => {
      const price =
        prices.get(balance.symbol?.toLowerCase()) ||
        prices.get(balance.address?.toLowerCase()) ||
        0;
  ...
  private calculateRiskScore(holdings: Holding[]): number {
    ...
    const hasHighVolatility = holdings.some(
      (h) => !h.tags.includes('STABLECOIN') && h.allocation > 10,
    );
  ...
  private calculatePeriodPerformance(data: any, holdings: Holding[]): PerformancePeriod {
  ...
  private generateRecommendations(
    ...
    const unstaked = holdings.filter(
      (h) =>
