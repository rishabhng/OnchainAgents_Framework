...
export class NFTValuator extends BaseAgent {
  ...
  protected async performAnalysis(context: AgentContext): Promise<NFTValuationResult> {
    ...
    const [collectionData, rarityData, marketData, salesHistory, holderData, liquidityData] =
      await Promise.all([
    ...
    const valuation = this.calculateValuation(
      ...
      marketData,
      salesHistory,
      context.options?.valuationMethods as string[] | undefined,
    );
  ...
  private findComparableNFTs(rarityScore: number, salesHistory: any): ComparableNFT[] {
    ...
    return sales
      .filter((s: any) => Math.abs(s.rarityScore - rarityScore) <= scoreRange)
      .map((s: any) => ({
        tokenId: s.tokenId,
  ...
  private calculateValuation(
    ...
    if (useHybrid) {
      ...
      estimatedValue = rarityValue * 0.4 + comparableValue * 0.4 + incomeValue * 0.2;
  ...
  private calculateIncomeValue(market: any, _collection: CollectionInfo): number {
  ...
  private calculateValuationConfidence(comparables: number, sales: number, volume: number): number {
  ...
  private analyzeMarketMetrics(market: any, sales: any): MarketMetrics {
    const recentSales =
      sales.sales?.filter((s: any) => this.calculateDaysAgo(s.timestamp) <= 1) || [];
  ...
  private assessLiquidity(liquidity: any, market: any): LiquidityProfile {
    ...
    const spread =
      asks[0]?.price && bids[0]?.price ? (asks[0].price - bids[0].price) / bids[0].price : 0;
  ...
  private generateTradingSignals(
    ...
    // Flip opportunity
    if (valuation.confidence > 70 && liquidity.liquidityScore > 50 && market.floorChange24h < -10) {
      signals.push({
        type: 'FLIP',
        strength: 8,
    ...
    // Hold signal
    if (rarity.rarityScore > 80 && market.whaleActivity === 'HIGH' && signals.length === 0) {
      signals.push({
        type: 'HOLD',
        strength: 9,
  ...
  private generateRecommendation(
    ...
    if (valuation.estimatedValue > valuation.floorPrice * 1.2) {
      reasoning.push(
        `Undervalued by ${Math.round((valuation.estimatedValue / valuation.floorPrice - 1) * 100)}%`,
      );
    }
