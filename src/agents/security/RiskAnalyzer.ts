...
export class RiskAnalyzer extends BaseAgent {
  ...
  private async calculateConcentrationRisk(portfolio: any): Promise<ConcentrationRisk> {
    ...
    const top3PositionsPercentage = (
      sortedAssets.slice(0, 3).reduce((sum, asset) => sum + asset.value, 0) / totalValue
    ) * 100;
    ...
    for (const asset of portfolio) {
      ...
      sectorConcentration[sector] = (sectorConcentration[sector] || 0) + (asset.value / totalValue) * 100;
    }
  ...
  private async calculateCorrelationRisk(portfolio: any, context: AgentContext): Promise<CorrelationRisk> {
    ...
    const correlationData = response.data as any;
  ...
  private async calculateVolatilityMetrics(portfolio: any, context: AgentContext): Promise<VolatilityMetrics> {
  ...
  private generateRecommendations(
    ...
    if (concentration.riskLevel === 'HIGH') {
      recommendations.push(`🚨 URGENT: Reduce ${concentration.largestAsset} exposure from ${concentration.topPositionPercentage.toFixed(1)}% to <25%`);
