...
export class IntrospectionEngine extends EventEmitter {
  ...
  private scoreAlternative(alternative: any): number {
    ...
    return Math.max(0, Math.min(1, 
      0.5 + (prosScore * 0.1) - (consScore * 0.15)
    ));
  }
  ...
  private calculateDecisionConfidence(
    ...
    if (alternatives.length === 0) {
      ...
      const bestAlternativeScore = Math.max(
        ...alternatives.map(a => this.scoreAlternative(a))
      );
  ...
  private identifyImprovements(): string[] {
    ...
    if (this.currentChain) {
      const lowConfidenceEntries = this.currentChain.entries
  ...
  private assessThinkingQuality(): MetaCognitiveAssessment {
    ...
    // Calculate metrics
    const avgConfidence = this.currentChain.entries.reduce(
    ...
    // Detect biases
    const biases = Array.from(this.patterns.values())
      .filter(p => p.type === 'bias');
  ...
  public getStatistics(): any {
    const completedChains = Array.from(this.chains.values())
      .filter(c => c.endTime);
    ...
    return {
      ...
      averageChainLength: completedChains.length > 0 ?
        completedChains.reduce((sum, c) => sum + c.entries.length, 0) / completedChains.length : 0,
      patternsDetected: this.patterns.size,
      biasesIdentified: Array.from(this.patterns.values())
        .filter(p => p.type === 'bias').length,
      knowledgeGaps: Array.from(this.patterns.values())
        .filter(p => p.type === 'gap').length,
