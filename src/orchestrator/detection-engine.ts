...
export class DetectionEngine extends EventEmitter {
   ...
   */
  private suggestPersonas(domains: CryptoDomain[], operations: OperationType[]): string[] {
   ...
   */
  private suggestAgents(domains: CryptoDomain[], _operations: OperationType[]): string[] {
    const agents: string[] = [];
  ...
  private shouldUseParallel(domains: CryptoDomain[], operations: OperationType[]): boolean {
    // Multiple domains benefit from parallel processing
    if (domains.length > 2) return true;

    // Certain operations benefit from parallelization
    const parallelOps = [OperationType.SCANNING, OperationType.MONITORING, OperationType.DISCOVERY];
   ...
   */
  public updateMetrics(tool: string, args: Record<string, any>, success: boolean): void {
    ...
    if (this.learningCache.has(key)) {
      ...
      cached.confidence = Math.max(
        0.1,
        Math.min(1.0, cached.confidence + (success ? 0.05 : -0.05)),
      );
    }
  }
  ...
  public getStatistics(): {
    ...
    return {
      ...
      averageComplexity:
        results.reduce((sum, r) => sum + r.complexity, 0) / Math.max(1, results.length),
