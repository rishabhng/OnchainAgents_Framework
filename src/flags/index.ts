...
const FLAG_DEFINITIONS: Map<string, FlagConfig> = new Map([
  ...
  [
    '--think-hard',
    {
      ...
      autoActivation: {
        condition: (ctx: any) =>
          ctx.complexity > 0.7 && ctx.operations?.includes(OperationType.SCANNING),
        confidence: 0.85,
      },
    },
  ],
  ...
  [
    '--hive',
    {
      ...
      autoActivation: {
        condition: (ctx: any) =>
          ctx.domains?.includes(CryptoDomain.WHALE) || ctx.domains?.includes(CryptoDomain.ALPHA),
        confidence: 0.85,
      },
    },
  ],
  ...
  [
    '--wave-mode',
    {
      ...
      autoActivation: {
        condition: (ctx: any) =>
          ctx.complexity >= 0.7 && ctx.fileCount > 20 && ctx.operationTypes > 2,
        confidence: 0.85,
      },
    },
  ],
  ...
  [
    '--loop',
    {
      ...
      autoActivation: {
        condition: (ctx: any) =>
          ctx.keywords?.includes('improve') || ctx.keywords?.includes('refine'),
        confidence: 0.8,
      },
    },
  ],
...
export class FlagManager extends EventEmitter {
   ...
   */
  public parseFlags(input: string | string[] | Record<string, any>): Map<string, any> {
  ...
  private async activateFlag(
    ...
    if (config.requiresDomain && context?.domains) {
      const hasRequiredDomain = config.requiresDomain.some((d) => context.domains.includes(d));
  ...
  public getStatistics(): {
    ...
    for (const activation of this.flagHistory) {
      ...
      // Flag frequency
      flagCounts.set(activation.flag, (flagCounts.get(activation.flag) || 0) + 1);

      // Category distribution
      const config = FLAG_DEFINITIONS.get(activation.flag);
      if (config) {
        categoryDist[config.category] = (categoryDist[config.category] || 0) + 1;
      }
    }
    ...
    const avgTokenImpact =
      impacts.length > 0 ? impacts.reduce((sum, i) => sum + i.token, 0) / impacts.length : 0;

    const avgPerformanceImpact =
      impacts.length > 0 ? impacts.reduce((sum, i) => sum + i.performance, 0) / impacts.length : 0;
