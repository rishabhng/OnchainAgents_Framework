...
import { ResourceZoneManager, ResourceZone } from '../orchestrator/resource-zones';
...
export class EmergencyProtocolManager extends EventEmitter {
  ...
  private async detectEmergencies(): Promise<void> {
    ...
    const emergencies: Array<{ type: EmergencyType; severity: EmergencySeverity; trigger: string }> = [];
    
    // Flash crash detection
    if (Math.abs(this.metrics.priceChange1m) > this.config.detectionThresholds.priceDropThreshold1m) {
      emergencies.push({
    ...
    if (Math.abs(this.metrics.priceChange1h) > this.config.detectionThresholds.priceDropThreshold1h) {
      emergencies.push({
    ...
    const highCorrelation = Array.from(this.metrics.correlations.values())
      .some(corr => corr > this.config.detectionThresholds.correlationThreshold);
  ...
  private createNotifications(
    ...
    // Determine priority
    const priority = severity === EmergencySeverity.CRITICAL ? 'urgent' :
                    severity === EmergencySeverity.HIGH ? 'high' : 'normal';
  ...
  private determineSystemAdjustments(
    ...
    return {
      circuitBreakers,
      degradationLevel,
      resourceLimit: severity === EmergencySeverity.CRITICAL ? 10 :
                    severity === EmergencySeverity.HIGH ? 30 :
                    severity === EmergencySeverity.MEDIUM ? 60 : 80,
      disabledFeatures,
    };
  }
  ...
  private createRecoveryPlan(
    ...
    const baseTime = severity === EmergencySeverity.CRITICAL ? 3600000 :  // 1 hour
                    severity === EmergencySeverity.HIGH ? 1800000 :      // 30 minutes
                    severity === EmergencySeverity.MEDIUM ? 900000 :     // 15 minutes
                    300000;                                               // 5 minutes
  ...
  private estimateImpact(
    ...
    const severityMultiplier = 
      severity === EmergencySeverity.CRITICAL ? 1000000000 :  // $1B
      severity === EmergencySeverity.HIGH ? 100000000 :       // $100M
      severity === EmergencySeverity.MEDIUM ? 10000000 :      // $10M
      1000000;                                                 // $1M
