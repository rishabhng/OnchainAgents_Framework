...
export class PersonaActivationEngine extends EventEmitter {
  ...
  private calculateKeywordScore(keywords: string[]): number {
    ...
    const highValueKeywords = [
      'whale', 'security', 'audit', 'yield', 'alpha',
      'exploit', 'vulnerability', 'opportunity', 'urgent',
    ];
  ...
  private calculatePerformanceScore(context: ActivationContext): number {
    ...
    const successfulActivations = recentActivations.filter(a => 
      a.selectedPersona && a.confidence > 0.8
    ).length;
  ...
  private matchesRule(rule: ActivationRule, context: ActivationContext): boolean {
    ...
    if (patterns.complexity) {
      if (context.complexity < patterns.complexity.min || 
    ...
    if (patterns.riskLevel) {
      if (context.riskLevel < patterns.riskLevel.min || 
  ...
  private applyUserPreferences(
    ...
    if (preferences.riskTolerance === 'low') {
      ...
      const conservativePersonas = [
        PersonaType.SECURITY_AUDITOR,
        PersonaType.RISK_MANAGER,
      ];
      ...
      const aggressivePersonas = [
        PersonaType.ALPHA_SEEKER,
        PersonaType.MARKET_MAKER,
      ];
  ...
  public getStatistics(): {
    ...
    const totalConfidence = this.activationHistory.reduce(
      (sum, a) => sum + a.confidence, 0
    );
    const averageConfidence = totalActivations > 0 ? 
      totalConfidence / totalActivations : 0;
