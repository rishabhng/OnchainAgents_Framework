...
export class CircuitBreaker<T = any> extends EventEmitter {
  ...
  private evaluateCircuitHealth(): void {
    ...
    const slowCalls = recentRequests.filter(r => r.duration > this.config.slowCallDuration).length;
...
export class CircuitBreakerManager extends EventEmitter {
  ...
  private checkCascadingFailure(): void {
    const openCircuits = Array.from(this.circuits.values())
      .filter(c => c.getState() === CircuitState.OPEN);
  ...
  public getStatistics(): any {
    ...
    return {
      ...
      halfOpenCircuits: Array.from(states.values()).filter(s => s === CircuitState.HALF_OPEN).length,
